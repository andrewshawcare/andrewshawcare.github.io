...
class AIClient:
    ...
    def analyze_code(self, diff, comments):
        headers = {
    ...
    for comment in comments:
        try:
            ...
            if 1 <= comment['line'] <= total_lines:
                ...
                print(f"Adding review comment: {comment}")
...
def main():
    ...
    try:
        ...
        diff = subprocess.check_output(
            ['git', 'diff', '--unified=0', pr.base.sha, pr.head.sha] + exclusions
        ).decode('utf-8')
        
        comments = pr.get_review_comments()

        feedback = ai_client.analyze_code(diff, comments)
