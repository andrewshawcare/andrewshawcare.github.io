...
const transformRelativeMarkdownLinksIntoRelativeHtmlLinks = (
  node: Markdoc.Node,
) => {
  if (
    node.type === "link" &&
...
export default class MarkdocWebpackPlugin implements WebpackPluginInstance {
  ...
  constructor(options: MarkdocWebpackOptions) {
    const contentPaths = globSync(options.content.pattern, {
      cwd: options.content.dir,
    }).map((pathOrString) => pathOrString.toString());

    this.htmlWebpackPlugins = contentPaths.map((contentPath) => {
      const resolvedContentPath = path.resolve(
        options.content.dir,
        contentPath,
      );
      ...
      const renderableTreeNode = Markdoc.transform(
        markdocNode,
        options.markdocConfig,
      );
      const renderedContent = Markdoc.renderers.html(renderableTreeNode);

      const filename = path.format({
        ...path.parse(contentPath),
        base: undefined,
        ext: ".html",
      });
  ...
  apply(compiler: Compiler): void {
    this.htmlWebpackPlugins.map((htmlWebpackPlugin) =>
